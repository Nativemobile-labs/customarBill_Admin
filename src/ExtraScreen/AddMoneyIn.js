...
import { moneyIn } from '../redux/reducerSlice.js/MoneyInSlice';


export default function AddMoneyIn({navigation}) {
...
const dispatch = useDispatch();
  ...
  // Show Date
    ...
    setMoneyInDate(date);
    let RecNo = 'Rec_' + (new Date()).getTime();
    setReceiptNo(RecNo);
  }, []);
  ...
  const handleSaveButton = () => {
    ...
    dispatch(moneyIn({receiptNo, moneyInDate, selectedState, totalAmount,chequeNo, chequeAmount,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Receipt Number */}
        ...
        {/* Select Sale Person */}
            <View style={{marginHorizontal: 20, marginTop: 15, flexDirection: 'row'}}>
                <Text style={{color: 'black', fontWeight: 'bold', fontSize: 18}}>Select Sale Person</Text>
                     <SelectDropdown 
                        buttonStyle={{height: 30, width: 100, marginRight: 10, backgroundColor: 'white'}}
                        buttonTextStyle={{fontSize: 14, textAlign: 'justify', color: 'black', fontWeight: '600'}}
                        rowStyle={{backgroundColor: 'white', borderBottomColor: 'silver'}}
                        rowTextStyle={{color: 'black', textAlign: 'justify', fontSize: 14, fontWeight: '600'}}
                        data={selectPerson}
                        defaultButtonText={"Admin"}
                        dropdownIconPosition={'right'}
                        renderDropdownIcon={salePerson => 
                                {return <Icons name={salePerson ? 'caret-down-outline' : 'caret-up-outline'} 
                                color={'#444'} size={14} /> }}
                        onSelect={(item) => {setSalePerson(item)}}
                     />
                </View>
          ...
          <View style={styles.mainView}>
            ...
            <TouchableOpacity style={styles.customerNameButton}>
              ...
              <Text style={{color: 'black', marginTop: 17, fontSize: 16}} onPress={() => navigation.navigate('AddCustomerToBill')}>Cash Sale</Text>
            </TouchableOpacity>
        ...
        {/* Mode of Transaction */}
        <Text style={{marginHorizontal: 30, color: 'black', fontWeight: '600', marginTop: 15}}>Mode of Txn</Text>
        ...
        {/* Cheque Payment */}
       {checked == 'Cash' ? null :<View style={{marginHorizontal: 20, marginTop: 15}}>
      ...
      <TouchableOpacity style={styles.saveButton} 
            onPress={() => [handleSaveButton(), navigation.navigate('Money In List')]}>
